...
const specimens = [
  ...
  { name: "text-body", className: "text-body", sample: "Sphinx of black quartz, judge my vow." },
  { name: "text-body-bold", className: "text-body-bold", sample: "Sphinx of black quartz, judge my vow." },
  { name: "text-caption", className: "text-caption", sample: "Pack my box with five dozen liquor jugs." },
];
  ...
  return (
    ...
    >
      <header className="mb-4 flex items-center justify-between">
        <h2 className="font-uni font-[700] text-lg tracking-tight">{title}</h2>
        <span
          className={[
            ...
            <div className="mb-1 flex items-center gap-2">
              <code
                className={[
                  ...
                  inverse ? "bg-surface-brown-1 text-paper-0" : "bg-paper-2 text-surface-ink",
                ].join(" ")}
              >
                .{s.name}
              </code>
      ...
      <section className="lg:col-span-2 rounded-2xl bg-paper-main ring-1 ring-paper-2 p-6">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <div className="rounded-xl bg-paper-0 p-4 ring-1 ring-paper-2">
            <h4 className="font-uni font-[700] mb-2">A) Preconfigured classes</h4>
            <p className="text-body mb-3">
              Use the named utilities you defined in <code>@layer components</code>:
            </p>
...
<span className="text-caption">Caption text…</span>`}
          ...
          <div className="rounded-xl bg-paper-0 p-4 ring-1 ring-paper-2">
            <h4 className="font-uni font-[700] mb-2">B) Manual font-family (advanced)</h4>
            <p className="text-body mb-3">
              If you need to bypass the preconfigured classes, you can target the raw font variables
              from <code>next/font</code>. In Tailwind v4 you already mapped them in <code>@theme</code>:
            </p>
            <ul className="text-caption mb-3 list-disc pl-5">
              <li><code>font-akz</code> → <code>var(--font-akzidenz)</code> stack</li>
              <li><code>font-uni</code> → <code>var(--font-univers)</code> stack</li>
            </ul>
            <pre className="overflow-x-auto rounded-md bg-surface-grey/10 p-3 text-caption">
{`// Using Tailwind tokens (preferred)
<h1 className="font-akz font-[900] tracking-[-2.5%] uppercase">Akzidenz Black</h1>
<p className="font-uni">Univers Regular</p>
<strong className="font-uni font-[700]">Univers Bold</strong>

// Or, inline style with the raw CSS variable
<div style={{ fontFamily: "var(--font-akzidenz), ui-sans-serif, system-ui, sans-serif" }}>
  Raw Akzidenz via CSS var
</div>`}
            </pre>
            <p className="text-caption mt-2 opacity-70">
              Tip: stick to the preconfigured classes for consistency; drop to manual only for edge cases.
            </p>
          </div>
        </div>
      </section>
